...
function InfoRow({ label, value }: InfoRowProps) {
  return (
    <>
      <div className="py-1.5 pr-2 font-medium text-muted-foreground text-sm break-words">{label}</div>
      <div className="col-span-2 py-1.5 pl-2 text-card-foreground text-sm break-words">{value}</div>
    </>
  );
}
...
export function BrowserInfoCard() {
  ...
  return (
    ...
    <div className="bg-card p-6 border rounded-lg text-card-foreground">
      <div className="mb-4">
        <h3 className="font-semibold text-xl tracking-tight">Browser Information</h3>
        <p className="mt-1 text-muted-foreground text-sm">Details about your Elo Browser installation.</p>
      </div>
