...
export function UpdateCard() {
  ...
  const renderStatusIconAndText = () => {
    ...
    if (downloadFailed) {
      return (
        <>
          ...
          <span className="text-destructive text-sm">Download failed for v{availableVersion}</span>
        </>
      );
    }
    if (isDownloadingUpdate) {
      return (
        <>
          ...
          <span className="text-primary text-sm">Downloading v{availableVersion}...</span>
        </>
      );
    }
    if (isDownloaded) {
      return (
        <>
          ...
          <span className="text-green-500 text-sm">Update v{availableVersion} downloaded</span>
        </>
      );
    }
    if (hasUpdate) {
      return (
        <>
          ...
          <span className="text-primary text-sm">Update available: v{availableVersion}</span>
        </>
      );
    }
    if (isUpToDate) {
    ...
    return (
      <>
        ...
        <span className="text-muted-foreground text-sm">Check for browser updates</span>
      </>
    );
  };
  ...
  const renderActionButton = () => {
    ...
    if (hasUpdate && isAutoUpdateSupported) {
      return (
        <Button variant="default" size="sm" onClick={downloadUpdate} className="min-w-[140px]">
          <ArrowDownToLine className="mr-2 w-4 h-4" /> Download v{availableVersion}
        </Button>
      );
    }
    ...
    return (
      ...
      >
        <RefreshCw className="mr-2 w-4 h-4" /> {isUpToDate ? "Check Again" : "Check for Updates"}
      </Button>
    );
  };
  ...
  return (
    <div className="bg-card p-6 border rounded-lg text-card-foreground remove-app-drag">
      <div className="mb-4">
        ...
        <p className="mt-1 text-muted-foreground text-sm">Current Version: {state.currentVersion}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex sm:flex-row flex-col justify-between items-start sm:items-center gap-3 bg-muted/30 p-3 border rounded-md">
          <div className="flex flex-grow items-center">{renderStatusIconAndText()}</div>
