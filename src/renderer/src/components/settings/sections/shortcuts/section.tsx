...
export function ShortcutsSettings() {
	...
	const { groupedKeybinds, modifiedShortcuts, sortedEntries } = useMemo(() => {
		...
		const modified = filtered.filter(
			(kb) =>
				!!kb.originalShortcut &&
				!!kb.shortcut &&
				kb.originalShortcut !== kb.shortcut,
		);
	...
	return (
		<div className="space-y-6 remove-app-drag">
			...
			>
				<h2 className="flex items-center gap-2 font-semibold text-card-foreground text-2xl">
					<KeyboardIcon className="w-6 h-6" />
					Keyboard Shortcuts
				</h2>
				<p className="text-muted-foreground">
					Customize Elo keyboard shortcuts to improve your workflow.
				</p>
			</motion.div>

			<motion.div
				className="space-y-6 bg-card p-6 border rounded-lg text-card-foreground"
			...
			<ResetDialog
				...
				onConfirm={performResetAllKeybinds}
			/>
		</div>
	);
}
