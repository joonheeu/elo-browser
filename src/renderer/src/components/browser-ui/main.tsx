...
function InternalBrowserUI({
	isReady,
	type,
}: { isReady: boolean; type: WindowType }) {
	...
	return (
		<MinimalToastProvider sidebarSide={side}>
			<ActionsProvider>
				{dynamicTitle && <title>{`${dynamicTitle} | Elo`}</title>}
				...
				<SidebarInset className="bg-transparent">
					<div
						className={cn(
							...
							type === "popup" &&
								"pt-[calc(env(titlebar-area-y)+env(titlebar-area-height))]",
						)}
					>
						{/* Topbar */}
						<div className="top-0 left-0 absolute flex justify-center items-center w-full h-2">
							<AnimatePresence>
								{isActiveTabLoading && (
									<motion.div
										className="bg-gray-200/30 dark:bg-white/10 rounded-full w-28 h-1 overflow-hidden"
										initial={{ opacity: 0 }}
										animate={{ opacity: 1 }}
										exit={{ opacity: 0 }}
									...
									>
										<motion.div
											className="bg-gray-800/90 dark:bg-white/90 rounded-full h-full"
											initial={{ x: "-100%" }}
											animate={{ x: "100%" }}
											transition={{
												duration: 1,
						...
						<div className="flex flex-col flex-1 w-full h-full">
							<div className="remove-app-drag">
								{type === "popup" && (
									<SidebarAddressBar className="rounded-lg" />
								)}
							</div>
							<BrowserContent />
						</div>
