			...
			shapes.forEach((shape) => {
				...
				if (shape.type === "square" || shape.type === "triangle") {
					...
					if (elapsedRatio < 0.5) {
						...
						const progressInSecondHalf = Math.min(
							1,
							(elapsedRatio - 0.5) / 0.5,
						);
				...
				const colorMatch = shape.color.match(
					/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*[\d.]+)?\)/,
				);
...
export function OnboardingScreen({ children }: { children?: React.ReactNode }) {
	...
	// --- Render Component ---
	return (
		<div className="relative flex flex-col justify-center items-center bg-[#050A20] w-full h-screen overflow-hidden select-none app-drag">
			{/* Set document title */}
			<title>Onboarding | Elo Browser</title>

			{/* Canvas for drawing the animated shapes */}
			<canvas
				ref={canvasRef}
				// Width/height attributes are set dynamically for HiDPI support
				className="z-0 absolute inset-0" // Place behind other elements
			/>

			{/* Dynamic backdrop blur container (Optional: Performance heavy) */}
			{/* Positioned above canvas (z-10) but below content (z-20) */}
			<motion.div
				className="z-10 absolute inset-0 w-full h-full pointer-events-none" // No pointer events
				style={{
					backdropFilter: `blur(${blurIntensity}px)`,
					WebkitBackdropFilter: `blur(${blurIntensity}px)`,
					transition:
						"backdrop-filter 3s ease-in-out, -webkit-backdrop-filter 3s ease-in-out",
				}}
			...
			<motion.div
				...
				className="top-[-20%] left-[-10%] z-[5] absolute bg-radial from-[#0066FF]/40 via-[#0066FF]/20 to-transparent blur-[60px] rounded-full w-[70%] h-[70%] pointer-events-none"
				aria-hidden="true"
			/>
			...
			<motion.div
				...
				className="right-[-20%] bottom-[-30%] z-[5] absolute bg-radial from-[#0066FF]/30 via-[#0055DD]/15 to-transparent blur-[70px] rounded-full w-[80%] h-[80%] pointer-events-none"
				aria-hidden="true"
			/>

			{/* Container for the actual screen content */}
			{/* Positioned above all background effects (z-20) */}
			<div className="z-20 relative flex flex-col justify-center items-center p-4 w-full h-full">
